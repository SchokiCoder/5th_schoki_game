...
int main()
{
		...
		for (ul32_t y = 0; y < read_world.height; y++) {
			for (ul32_t z = 0; z < read_world.depth; z++) {
				...
				printf("x: %u, y: %u, z: %u -> %lu\n", x, y, z,
				       read_world.blocks[x][y][z]);*/

				if (read_world.blocks[x][y][z] !=
				    write_world.blocks[x][y][z]) {
	...
	for (ul32_t i = 0; i < read_world.ent_count; i++) {
		if (read_world.entities[i].id != write_world.entities[i].id
		    || read_world.entities[i].grounded !=
		    write_world.entities[i].grounded
		    || read_world.entities[i].velocity_x !=
		    write_world.entities[i].velocity_x
		    || read_world.entities[i].velocity_y !=
		    write_world.entities[i].velocity_y
		    || read_world.entities[i].rect.x !=
		    write_world.entities[i].rect.x
		    || read_world.entities[i].rect.y !=
		    write_world.entities[i].rect.y
		    || read_world.entities[i].rect.w !=
		    write_world.entities[i].rect.w
		    || read_world.entities[i].rect.h !=
		    write_world.entities[i].rect.h) {
